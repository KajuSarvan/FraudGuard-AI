...
def create_preset_invoice(req: PresetRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    ...
    invoice = Invoice(
        ...
        status="PENDING",
        reasoning="Preset demo invoice created.",
    )
    db.add(invoice)
    db.commit()
    db.refresh(invoice)
...
async def _run_live_pipeline(invoice_text: str, db: Session) -> Dict[str, Any]:
...
async def analyze_invoice_endpoint(req: AnalyzeRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        return await asyncio.wait_for(_run_live_pipeline(req.invoice_text, db), timeout=8.0)
    except Exception as e:
